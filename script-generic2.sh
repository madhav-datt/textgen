...
KERAS_BACKEND=theano THEANO_FLAGS=mode=FAST_RUN,device=gpu,floatX=float32 python src/train-wonderland-3.py -t $filename -w $weightfile -l $layers -n $nodes -e $epochs -b $batchsize -d $dropout